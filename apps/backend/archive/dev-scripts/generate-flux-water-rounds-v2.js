// Water type v2: Above-water arena (not submerged), no stance language on Titan attack
// Changes from v1:
//   - Arena: standing on water surface, splashing waves, not underwater
//   - Titan attack: removed "aggressive lunging attack stance"
//   - Output to same folders (test-flux-water*) — delete old files first
// Run: node generate-flux-water-rounds-v2.js
...
// Water type visuals — v2: ABOVE water, not submerged
...
async function main() {
  ...
  // Delete all existing water images first
  console.log('Cleaning old water images...');
  for (const round of ROUNDS) {
    const dir = path.join(BASE_DIR, round.folder);
    if (fs.existsSync(dir)) {
      const files = fs.readdirSync(dir).filter(f => f.endsWith('.webp'));
      for (const f of files) {
        fs.unlinkSync(path.join(dir, f));
        console.log(`  Deleted ${round.folder}/${f}`);
      }
    }
  }
  ...
  for (let r = 0; r < ROUNDS.length; r++) {
    ...
    console.log(`\n${'='.repeat(50)}`);
    console.log(`WATER ${round.stat.toUpperCase()} (v2 — above water)`);
  ...
  console.log(`\n${'='.repeat(50)}`);
  console.log(`ALL WATER v2 ROUNDS COMPLETE`);
